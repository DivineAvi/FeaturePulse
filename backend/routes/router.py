...
async def get_dashboard_data():
    ...
    try:
        competitors = db.get_competitors()
        changes = db.get_all_changes()  # Get all changes
...
async def get_changes(
    ...
    try:
        # If no competitor_id is provided, get all changes
        if competitor_id:
            changes = db.get_changes(competitor_id)
        else:
        ...
        transformed_changes = []
        for change in changes:
            # Get competitor name if we have competitor_id
            competitor_name = None
            if change.get("competitor_id"):
                competitors = db.get_competitors()
                competitor = next((c for c in competitors if c["id"] == change["competitor_id"]), None)
                if competitor:
            ...
            transformed_change = {
                ...
                "description": change.get("summary"),  # Map summary to description
        ...
        # Add debugging info
        print(f"DEBUG: Fetched {len(changes)} changes, transformed to {len(transformed_changes)}")
        if transformed_changes:
            print(f"DEBUG: First transformed change: {transformed_changes[0]}")
        
        return transformed_changes
    
    except Exception as e:
        print(f"DEBUG: Error in get_changes: {str(e)}")
